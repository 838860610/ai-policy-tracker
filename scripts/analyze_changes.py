...
功能：
  - 读取 data/update_status.json，找出 status=changed 的产品
  - 对每个变更产品，对比新旧政策正文快照：
      新快照：data/snapshots/{id}/{target}/latest.txt（本次检查抓取）
      旧快照：data/snapshots/{id}/{target}/ 下除 latest.txt 外最新的日期存档，
              或从 git 历史取（data/snapshots/ 已入 git 时）
  - 调用 LLM（OpenAI 兼容 API）对比新旧内容，提取关键变化点：
      - 训练数据政策是否变化
      - 退出/关闭机制是否变化
      - 数据留存期限是否变化
      - 其他关键条款变化
  - 生成结构化变更报告，写入 data/change_reports/{id}_{date}.json
...
输出文件：
  data/change_reports/{id}_{date}.json - 结构化变更分析报告
"""
...
REPORTS_DIR = os.path.join(BASE_DIR, "data", "change_reports")
...
def render_markdown_summary(reports):
    ...
    lines.append("3. 完整分析报告：`data/change_reports/` 目录下各产品 JSON 文件")
    lines.append("")
