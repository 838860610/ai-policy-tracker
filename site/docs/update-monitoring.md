...
`policies.google.com` 支持 `gl`（地区）与 `hl`（语言）参数，且 `gl` 会**覆盖 IP 推断**（实测本地加 `?gl=US&hl=zh-CN` 稳定返回美国版）。锁定后无论在哪个网络环境跑，抓到的都是同一份文本。

选 `gl=US` 而不是"国区"的理由是**监控实际运行在 GitHub 美国 runner 上**（`monitor.yml` 每周在那儿跑），锁 CI 的地区才能让本地与 CI 的基线一致；`hl=zh-CN` 则为与中文读者的定位统一——`hl` 的影响远大于 `gl`（`gemini/toc` 英文版 56422 字符 vs 中文版 24854 字符）。

**锁定前实测的三地区对比结论**（决定了"锁哪个地区都不改变本表记录"）：

| 目标 | CN vs SG | CN vs US |
| --- | --- | --- |
| `gemini/main`（通用服务条款） | 仅差页面上的"国家/地区版本"标签，正文一字不差 | 55 处 / 1562 字符（15%），均为翻译措辞差异 |
| `gemini/toc`（Gemini 隐私中心，**核心政策来源**） | 哈希**完全相同**（地区不敏感） | 仅差 29 字符 |

核心政策表述（默认 18 个月留存、3/36 个月选项、人工审核、训练新模型）三地区逐条一致。

> **改 URL 的代价**：URL 变化会触发 `source_changed` → 该目标报 `changed` 并进待核实队列，需人工确认新来源后 `--resolve` 重建基线。
> **在受限网络下**：本机可能完全连不上这些域名（Chromium 也会卡住），此时重建基线要靠 CI 的那一轮——别把本地"抓不到"产生的 `degraded` 状态提交上去，那会污染状态文件。
